 ...
 ***/
public class TelnetTerm implements Runnable, TelnetNotificationHandler
{
    ...
    public static void main(String[] argv) throws Exception
    {
        ...
        while (true) {
            ...
            try {
                ...
                Thread reader = new Thread (new TelnetTerm());
                tc.registerNotifHandler(new TelnetTerm());
                System.err.println("TelnetClientExample");
                System.err.println("Type AYT to send an AYT telnet command");
                System.err.println("Type OPT to print a report of status of options (0-24)");
                System.err.println("Type REGISTER to register a new SimpleOptionHandler");
                System.err.println("Type UNREGISTER to unregister an OptionHandler");
                System.err.println("Type SPY to register the spy (connect to port 3333 to spy)");
                System.err.println("Type UNSPY to stop spying the connection");
                    ...
                    try
                    {
                        ...
                        if(ret_read > 0)
                        {
                            if((new String(buff, 0, ret_read)).startsWith("AYT"))
                            {
                                try
                                {
                                    System.err.println("Sending AYT");

                                    System.err.println("AYT response:" + tc.sendAYT(5000));
                                }
                                catch (IOException e)
                                {
                            ...
                            {
                                 System.err.println("Status of options:");
                                 for(int ii=0; ii<25; ii++) {
                                     System.err.println("Local Option " + ii + ":" + tc.getLocalOptionState(ii) + " Remote Option " + ii + ":" + tc.getRemoteOptionState(ii));
                                 }
                            }
            ...
            {
                    System.err.println("Exception while connecting: " + e.getMessage()+", "+e.toString());
                    System.exit(1);
            }
        }
    }
    ...
    public void run()
    {
        ...
        try
        {
            ...
            do
            {
                ...
                if(ret_read > 0)
                {
                    System.err.print(new String(buff, 0, ret_read));
                }
            }
            while (ret_read >= 0);
        }
